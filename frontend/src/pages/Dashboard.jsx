...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on('newData', (newRecord) => {
      setData((prev) => [newRecord, ...prev].slice(0, 1000));
  ...
  const handleLocationSearch = async (locationItem) => {
    ...
    setData((prev) => [newReading, ...prev].slice(0, 1000));
  ...
  return (
    ...
    >
      ...
      {/* Main Table Section */}
      <div className="mt-6">
        <motion.div variants={itemVariants} className="glass-card overflow-hidden relative w-full">
