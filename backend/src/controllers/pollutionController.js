...
const getLivePollution = async (req, res) => {
    try {
        const data = await Pollution.find().sort({ timestamp: -1 }).limit(1000);
        res.json(data);
    } catch (error) {
...
const addPollutionData = async (req, res) => {
    try {
        ...
        const newPollution = new Pollution({
            zone: zone || 'Custom Zone', pm25, pm10, no2, co, temperature, humidity, vehicle_count, speed,
        ...
        if (prediction.risk_level === 'Hazardous' || prediction.risk_level === 'Very Unhealthy') {
            await logToBlockchain(zone || 'Custom Zone', prediction.aqi_1hr, prediction.risk_level);
        }

        req.io.emit('newData', newPollution);

        res.status(201).json({ message: 'Data added successfully', data: newPollution, prediction });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
