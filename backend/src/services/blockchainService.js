...
const logToBlockchain = async (zone, aqi, riskLevel) => {
    try {
        ...
        const intAqi = Math.round(aqi);
        const dataHash = ethers.id(`${zone}-${intAqi}-${timestamp}`);

        console.log(`Logging to blockchain: ${zone}, AQI: ${intAqi}`);
        const tx = await contract.addRecord(zone, intAqi, riskLevel, timestamp, dataHash);
        await tx.wait();
